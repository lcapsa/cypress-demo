...
describe("GROUPS - CHAT MESSAGES - REPORT", function () {
  before(function () {
    cy.log(
  ...
  it("Message is successfully reported @SMOKE", function () {
    cy.log(
      "STEP 1: User " +
        this.studentUaId +
    ...
    chatMessagesRequest
      ...
      .then((response) => {
        ...
        chatMessagesRequest
          .reportMessage(this.professorAtToken, messageId)
          .then((response) => {
            expect(response.status).to.equal(200);
            ...
            chatMessagesRequest
              ...
              .then((response) => {
                expect(response.status).to.equal(200);
                chatRoomMessagesResponse.verifyMessageIsReported(
                  response.body,
                  messageId,
                  this.studentUaId,
